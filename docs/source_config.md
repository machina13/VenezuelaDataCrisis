...
sources:
  - id: encuentralos_tecnosoft
    ...
    max_concurrent_posts: 8  # deprecated; el exporter actual usa batch PostgREST
...
| `max_concurrent_posts` | no | Deprecated. Si aparece en YAML, el pipeline loguea warning y lo ignora; el exporter actual usa batch PostgREST. |
...
| `bulk_size` | no | Entero positivo: cuántos aportes enviar por batch a Supabase/PostgREST. Si se omite, el exporter usa su default conservador. |
...
- Si `bulk_size` está presente, debe ser un entero positivo; controla el tamaño del batch PostgREST
